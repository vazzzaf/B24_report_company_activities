...
filename = 'CompletedActivities_' + today + '.xlsx'
...
def formatSheet(worksheet):
    ...
    worksheet.set_column("D:D", 12, text_format)
    worksheet.set_column("E:E", None, text_format)
    worksheet.set_column("F:G", 18, text_format)
    worksheet.set_column("H:I", 28, text_format)
    worksheet.set_column("J:J", 14, text_format)
    worksheet.set_column("K:K", 14, link_format)
    worksheet.set_column("L:L", None, text_format)
    worksheet.set_column("M:M", 28, text_format)
    worksheet.set_column("N:O", 28, link_format)
...
if len(fileData) > 0:
    ...
    fileData.to_excel(writer, 'CreatedALL', index=False)
    
    workbook  = writer.book
    worksheet = writer.sheets['CreatedALL']
    formatSheet(worksheet)
    
# Созданные и выполненные
fileData = df[df['Completed']=='YES']

if len(fileData) > 0:
    # Write your DataFrame to a file     
    fileData.to_excel(writer, 'Completed', index=False)
    
    workbook  = writer.book
    worksheet = writer.sheets['Completed']
    formatSheet(worksheet)
    
# Созданные и не просроченные
fileData = df[df['Deadline']>dt.now()]

if len(fileData) > 0:
    # Write your DataFrame to a file     
    fileData.to_excel(writer, 'CompletedWithoutExpired', index=False)
    
    workbook  = writer.book
    worksheet = writer.sheets['CompletedWithoutExpired']
    formatSheet(worksheet)

writer.save()

send_mail('-------------------------', send_to, 'AllTasks ' + today, 'Файл в приложении.', [ './' + filename ])
os.remove('./' + filename)
